...
rest_areas = ti.field(ti.f32, shape=N_triangles)

# We also need to draw the edges
edges_set = set()
for i in range(N_triangles):
    edges_set.add((faces[i][0], faces[i][1]) if faces[i][0]
                  < faces[i][1] else (faces[i][1], faces[i][0]))
    edges_set.add((faces[i][1], faces[i][2]) if faces[i][1]
                  < faces[i][2] else (faces[i][2], faces[i][1]))
    edges_set.add((faces[i][2], faces[i][0]) if faces[i][2]
                  < faces[i][0] else (faces[i][0], faces[i][2]))
...
D = ti.Matrix.field(2, 2, dtype=ti.f32, shape=N_triangles)
F = ti.Matrix.field(2, 2, dtype=ti.f32, shape=N_triangles)
stress = ti.Matrix.field(2, 2, dtype=ti.f32, shape=N_triangles)
...
def compute_D(D):
    for i in triangles:
        v0, v1, v2 = triangles[i]
        x0, x1, x2 = x[v0], x[v1], x[v2]
        D[i] = ti.Matrix.cols([x1-x0, x2-x0])
...
def corotated_linear():
    for i in range(N_triangles):
        ...
        strain = S - ti.Matrix.identity(ti.f32, 2)
